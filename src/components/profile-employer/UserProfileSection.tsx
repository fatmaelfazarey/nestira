...
export function UserProfileSection({
  ...
  useEffect(() => {
    ...
    setBusinessEmail(currentUser?.personalInfo?.businessEmail);
    setProfilePhoto(currentUser?.personalInfo?.profilePhoto);
    setLinkedinPersonal(currentUser?.personalInfo?.linkedinPersonal || '');
    setWhatsappNumber(currentUser?.personalInfo?.whatsappNumber || '');
  }, [currentUser]);
    ...
    <div className="space-y-6">
        ...
        {isViewMode ? (
          <div className="p-3 bg-gray-50 rounded-md border border-gray-200">
            <span className="text-gray-900">
              {isIndividualRecruiter ? 'Individual Recruiter' : 'Company Representative'}
            </span>
          </div>
        ) : (
          <RadioGroup
          ...
          >
            <div className="flex items-center space-x-3 p-3 border rounded-lg hover:bg-gray-50">
              <RadioGroupItem value="individual" id="individual" disabled={isViewMode} />
              <div>
                <Label htmlFor="individual" className="font-medium cursor-pointer">
                  Individual Recruiter
                </Label>
            ...
            <div className="flex items-center space-x-3 p-3 border rounded-lg hover:bg-gray-50">
              <RadioGroupItem value="company" id="company" disabled={isViewMode} />
              <div>
                <Label htmlFor="company" className="font-medium cursor-pointer">
                  Company Representative
                </Label>
        ...
        <div className="space-y-2">
          ...
          {isViewMode ? (
            <div className="text-sm text-gray-600">
              {profilePhoto ? "Profile photo uploaded" : "No profile photo"}
            </div>
          ) : (
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" asChild>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          ...
          {isViewMode ? (
            ...
            <Input
              id="full-name"
              value={fullName}
              onChange={(e) => handleChangeWrapper(e.target.value, setFullName, 'fullName')}
              placeholder="Enter your full name"
              className="h-9"
            />
          )}
        ...
        <div className="space-y-2">
          ...
          {isViewMode ? (
            ...
            <Input
              id="role-position"
              value={rolePosition}
              onChange={(e) => handleChangeWrapper(e.target.value, setRolePosition, 'rolePosition')}
              placeholder={isIndividualRecruiter ? 'e.g., Finance Headhunter' : 'e.g., Senior Recruiter'}
              className="h-9"
            />
          )}
        ...
        <div className="space-y-2">
          ...
          {isViewMode ? (
            ...
            <Input
              id="phone-number"
              value={phoneNumber}
              onChange={(e) => handleChangeWrapper(e.target.value, setPhoneNumber, 'phone')}
              placeholder="+20 XXX XXX XXXX"
              className="h-9"
            />
          )}
        ...
        <div className="space-y-2">
          ...
          {isViewMode ? (
            <div className="p-3 bg-gray-50 rounded-md border border-gray-200">
              <span className="text-gray-900">{businessEmail || '-'}</span>
            </div>
          ) : (
            <Input
              id="business-email"
              type="email"
              value={businessEmail}
              onChange={(e) => handleChangeWrapper(e.target.value, setBusinessEmail, 'businessEmail')}
              placeholder={isIndividualRecruiter ? 'your.email@domain.com' : 'your.email@company.com'}
              className="h-9"
            />
          )}
        ...
        {isIndividualRecruiter && (
          <>
            <div className="space-y-2">
              ...
              {isViewMode ? (
                <div className="p-3 bg-gray-50 rounded-md border border-gray-200">
                  <span className="text-gray-900">{linkedinPersonal || '-'}</span>
                </div>
              ) : (
                <Input
                  id="linkedin-personal"
                  value={linkedinPersonal}
                  onChange={(e) => handleChangeWrapper(e.target.value, setLinkedinPersonal, 'linkedinPersonal')}
                  placeholder="https://linkedin.com/in/yourname"
                  className="h-9"
                />
              )}
            ...
            <div className="space-y-2">
              ...
              {isViewMode ? (
                <div className="p-3 bg-gray-50 rounded-md border border-gray-200">
                  <span className="text-gray-900">{whatsappNumber || '-'}</span>
                </div>
              ) : (
                <Input
                  id="whatsapp-number"
                  value={whatsappNumber}
                  onChange={(e) => handleChangeWrapper(e.target.value, setWhatsappNumber, 'whatsappNumber')}
                  placeholder="+971 50 123 4567"
                  className="h-9"
                />
              )}
