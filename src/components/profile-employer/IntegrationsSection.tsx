  ...
  return (
    <div className="flex items-center justify-between p-4 border rounded-lg bg-white/50 shadow-sm transition-all hover:shadow-md">
      ...
      {!isViewMode ? (
        ...
        <div className={`px-3 py-2 rounded-md ${isConnected ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600'}`}>
...
export function IntegrationsSection({ 
  ...
  const { isRTL } = useLanguage();
  const [connected, setConnected] = useState({
    meet: currentUser?.integrations?.googleMeet?.connected || false,
    calendar: currentUser?.integrations?.googleCalendar?.connected || false,
    gmail: currentUser?.integrations?.gmail?.connected || false,
  });
