...
export const FindMyMatchModal: React.FC<FindMyMatchModalProps> = ({
  ...
  const { getMatchedJobs } = useEmployerStore();
  ...
  const fetchMatchedJobs = async () => {
    const jobs = await getMatchedJobs(setActiveJobPosts);
    if (!jobs.success) {
      console.error('Filed to get matched jobs');
    }
  }
  return <Dialog open={isOpen} onOpenChange={onClose}>
    <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto space-y-4 mt-6 my-0">
        ...
        <Card key={job.id} className={`cursor-pointer transition-all duration-200 hover:shadow-md ${selectedJob?.id === job.id ? 'border-orange-500 bg-orange-50' : 'hover:border-gray-300'}`} 
