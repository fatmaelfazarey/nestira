...
const RecruitmentBoard = () => {
  ...
  const handleStartChat = (candidate: any) => {
    console.log('Start chat with:', candidate);
  ...
  const handleStageChange = async (candidate: any, jobApplicationId: number, newStageId: string) => {
    // console.log('---------candidate---------- =>', candidate)

    const isUpdated = await updateHiringStage(newStageId, jobApplicationId, candidate.userId, candidate.jobId);
    if (isUpdated.success && newStageId === 'interviewing') {
      // toast.success('inerview add sucessfully , please go to interview page to set interview time');
      toast.success("Interview added successfully. Please go to the interview page to set the interview time.", {
        duration: 5000,
        dismissible: true,
      });
    } else if (isUpdated.success) {
      toast.success('Hiring stage updated successfully');
    } else {
      toast.error('Failed to update hiring stage. Please try again.')
    }
    ...
    setStages(prevStages => {
      ...
      for (const stage of nextStages) {
        const candidateIndex = stage.candidates.findIndex(c => c.id === jobApplicationId);
        if (candidateIndex !== -1) {
          [candidateToMove] = stage.candidates.splice(candidateIndex, 1);
          break;
        }
                            ...
                            return (
                              <TableRow key={candidate.id} data-state={selectedCandidates.has(candidate.id) ? "selected" : ""}>
                                ...
                                <TableCell>
                                  ...
                                  <Select
                                    value={currentStageId}
                                    onValueChange={(newStageId) => handleStageChange(candidate, candidate.id, newStageId)}
                                  >
                                    <SelectTrigger className="w-[130px]">
                                      <SelectValue />
                                    </SelectTrigger>
                                ...
                                <TableCell>
                                  <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end">
                                      <DropdownMenuItem onClick={() => { handleOpenProfile(candidate.userId); }}>View full profile</DropdownMenuItem>
                                      <DropdownMenuItem>Add tag</DropdownMenuItem>
                                      {currentStageId !== 'rejected' && (
                                        <DropdownMenuItem
                                          className="text-red-600"
                                          onClick={() => handleStageChange(candidate, candidate.id, 'rejected')}
                                        >
                                          Mark as rejected
                                        </DropdownMenuItem>
                                      )}
